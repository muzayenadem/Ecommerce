...
function LoginDrowbar({}) {
    const [email, setEmail] = useState('')
    const [password, setPassword] = useState('')
 
    //const [isOpen, setIsOpen] = useState(false) 
   
    const isOpen = useSelector(state => state.loginState.isOpen)
    console.log(isOpen)
    // despatching
     ...
     try {   
      ...
      await axios.post('http://localhost:4300/login',{email,password})
       .then(()=>{
          ...
          //navigate('/home')
          //window.relaod = 'http://localhost:5173/home'
       })
       .catch((err)=>{
  ...
  return (
    ...
    {
        isOpen && (
            <div className='container origin-top-right absolute  top-40 -right-96 p-10 rounded-2xl w-96 mt-[100px] text-center shadow-lg bg-slate-800  place-items-center ring-1 ring-black ring-opacity-5'>
                <div
                className='py-1'
                role='menu'
