...
function ProfileDrawbar({opened,nopend}) {
    ...
    const logoutHandler = async () => {
      try {
        const response = await axios.get('http://localhost:4300/logout');
  ...
  return (
    ...
    <div className='flex'>
        ...
        {
          profile.image ?
            <img onMouseOver={toggleDropDown}  src={profile.image} className='w-10 h-10 rounded-full cursor-pointer' alt="" /> : 
            ...
            <div className='container z-10 origin-top-right absolute -top-10 -right-10 p-0 rounded-2xl w-96  mt-[100px]  shadow-lg bg-white place-items-center ring-1 ring-black ring-opacity-5'>
                ...
                >
                  <div>
                    ...
                    {
          profile.image ?
            <img onClick={toggleDropDown} src={profile.image} className='w-14 h-14 rounded-full cursor-pointer' alt="" /> : 
            <div className='w-14 h-14 rounded-full z-10 cursor-pointer capitalize bg-neutral-900 text-white font-bold text-center '>
             <h1 onClick={toggleDropDown} className='text-center text-2xl mt-3'> {profile.firstName[0]} {profile.lastName[0]}</h1>
                    ...
                    <hr></hr>
                    <p onClick={logoutHandler} className='block px-4 py-2  text-gray-700 hover:bg-gray-100 font-bold text-xl' >Sign out</p>
